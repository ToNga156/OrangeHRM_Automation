...
export class AssignLeavePage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        this.menuAssignLeave = page.getByRole('link', {name: 'Assign Leave'});
        ...
        this.btnAssign = page.getByRole('button', {name: 'Assign'});
        this.btnConfirmAssign = page.getByRole('button', {name: 'Ok'});
        this.toastSuccess = page.locator('.oxd-toast');
        this.lblRequiredEmployee = page.locator('.oxd-input-field-error-message').first();
        this.lblRequiredLeaveType = page.locator('.oxd-input-field-error-message').nth(1);
    ...
    async assignLeave(leave: Leave): Promise<void> {
        if (leave.employeeName) {
            await this.enterEmployeeName(leave.employeeName);
        }
    ...
    async verifyRequiredEmployee(): Promise<void> {
        await expect(this.lblRequiredEmployee).toHaveText('Required');
    }

    async verifyRequiredLeaveType(): Promise<void> {
        await expect(this.lblRequiredLeaveType).toHaveText('Required');
    }
